...
import { ExportRes, type RadarEvent } from '../../packages/common/src/index.js';
...
async function getJson(
  ...
  try {
    ...
    throw new Error(`${url.replace(/\?.*$/, '')} → ${res.status}: answer is not JSON`);
  }
}
...
export async function fetchAllEvents(
  ...
  for (;;) {
    const { status, body } = await getJson(
      `${server}/v1/events/export?from=${from}&limit=${pageSize}`,
      token,
      fetchImpl,
    );
    if (status < 200 || status >= 300) throw new Error(`GET /v1/events/export → ${status}`);
    // validated against the contract so a drifted server shape fails here, not as wrong numbers later
    const parsed = ExportRes.safeParse(body);
    if (!parsed.success)
      throw new Error(
        `GET /v1/events/export: answer does not match ExportRes (${parsed.error.issues[0]?.path.join('.') ?? '?'})`,
      );
    const page = parsed.data.events.filter((e) => e.id >= from);
    all.push(...page);
...
export async function collectRoundB(o: {
  server: string;
  token: string;
  repo?: string;
  base?: string;
  ...
  if (o.repo && o.base) {
    ...
    const shas = events.flatMap((e) =>
      e.type === 'commit.created' && e.payload.pushed ? [e.payload.sha] : [],
    );
...
export async function main(argv: string[], env: NodeJS.ProcessEnv = process.env): Promise<number> {
  ...
  if (!server || !out || !token) {
    console.error(
      'usage: RADAR_TOKEN=<mc|pm token> tsx scripts/ab/collect-round-b.ts --server <url> --out <round-b.json> [--events-out <file>] [--repo <dir> --base <sha>]',
    );
    return 2;
  }
  const repo = arg('--repo');
  const base = arg('--base');
  const r = await collectRoundB({
