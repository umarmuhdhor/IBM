...
1. LANGKAH MANUAL: Bob Shell P1 sudah terbuka di terminal app dengan Node 24. Setelah login browser pengguna, layar `IBM License Agreement` muncul; pengguna perlu meninjau dan memilih sendiri sebelum uji respons "halo" (rincian di bawah). Launcher IBM Bob pada macOS memilih Node 24 lewat nvm; perintah `bob` langsung di shell Node 20 tetap gagal. Mock server fase 02 belum tersedia di checkout ini.
...
1. Bob Shell sudah mencapai `IBM License Agreement` setelah pengguna menyelesaikan sign-in browser. Pengguna meninjau dokumen lisensi dengan Enter/panah, lalu memilih sendiri `y` bila setuju atau `n` bila menolak. Jangan kirim kredensial atau token ke repo/chat. Bob IDE tetap jalur P0; ini verifikasi P1 DA-02.
2. Jika lisensi diterima, kembali ke terminal Bob Shell dan kirim pesan `halo`; catat apakah Bob merespons. Untuk menguji launcher otomatis sesudah itu, buat/buka worktree, pilih agent **IBM Bob** dari pemilih agent (bukan mengetik `bob` langsung), lalu pastikan Bob Shell terbuka dengan Node 24.
