...
- **Cara membangunnya:** 4 orang, 4 lane paralel, masing-masing dengan AI sendiri (Claude Code + plugin **ECC**). Semua memakai satu `PROMPT.md` dan cukup mengganti 2 baris (`LANE`, `FASE`).
...
| Hook blokir di Bob IDE | Kemungkinan besar ya, dibuktikan di spike 1 | Docs lifecycle hooks Bob: `PreToolUse` exit 2 memblokir, config `.bob/settings.json`. Tapi stderr hanya masuk log, jadi pesan blokir belum tentu sampai ke model (R3 §2.2). | Kalau hook di IDE tidak jalan, penegakan tetap ada di server + sync agent (lapis 2). Pesan ke model lewat jalur fallback R3 §2.2 (instruksi mode → `why_blocked`). |
| Server tanpa VPS | Ya | **Cloudflare Workers + Durable Objects** (plan Free): WebSocket Hibernation, SQLite bawaan, satu DO per workspace. Commit lewat GitHub Git Data API (R4 §6.3, 4 subrequest per commit dengan isi file inline di tree; ≤ 100 file / 5 MB per task). Web di Cloudflare Pages. Detail di `ARCHITECTURE.md` §3. | Tim belum familiar DO → plugin resmi `cloudflare@cloudflare` (skill `durable-objects`). Cadangan: `wrangler dev` + Cloudflare Tunnel. |
...
Empat orang, empat lane, semua berjalan paralel. **Batas lane = batas folder**, supaya merge hampir tidak pernah bentrok.
...
**Kontrak** (`radar/packages/common/**`, `plan/ref/**`) hanya diubah oleh **Lane Alief** lewat "contract PR". Lane Umar, Aarief, dan Imelda yang butuh perubahan menulis entri `DECISIONS.md` (prefix `D-umar-..`/`D-aarief-..`/`D-imelda-..`) lalu mention Lane Alief.
...
| `lane/web` | Lane Imelda: landing + replay web + media | Imelda |
...
|---|---|
| Alief | Property test invariant kunci (R4), `bench-sync`, `sim-3pc` skenario tambahan, endpoint `/v1/files/history` |
| Umar | Tambahan fixture payload hook, test MCP per role, prompt siap tempel `bob-kit/prompts/*`, draft `BOB_DEVELOPMENT.md` |
| Aarief | Kondisi kosong & error semua komponen `@radar/ui`, test komponen, polish panel Live Collab |
| Imelda | Halaman `radar/packages/web/app/gallery` untuk semua komponen `@radar/ui`, replay dari fixture baru |
...
|---|---|---|
| `main` | integrasi (hanya fase 00 di-commit langsung; sisanya lewat PR) | – |
...
| `lane/<x>-fNN` | pemilik lane | snapshot ujung fase NN, hanya untuk PR (dihapus saat merge) |

**Alur per fase (AI menjalankannya lewat `plan/PROMPT.md` langkah 3, 11, 13)**
...
|---|---|---|
| App desktop | `IBM Bob Live Collab.dmg` di GitHub Releases → seret ke Applications → System Settings → Privacy & Security → Open Anyway (tidak di-sign; klik kanan → Open tidak berlaku lagi sejak macOS 15) | P0 |
