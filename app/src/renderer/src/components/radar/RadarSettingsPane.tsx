...
export function RadarSettingsPane({ connection, connected, workspacePath, onConnectionChange }: Props) {
  ...
  const disconnect = async () => {
    ...
    try {
      ...
      setMessage('Could not remove the connection.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <form aria-label="Live Collab settings" className="max-w-lg space-y-3 p-4" onSubmit={(event) => { event.preventDefault(); void connect() }}>
      ...
      <label className="block text-xs">Server URL<input required type="url" value={server} onChange={(event) => setServer(event.target.value)} placeholder="http://localhost:3000" className="mt-1 w-full rounded-md border border-border bg-card p-2" /></label>
