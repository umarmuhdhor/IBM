...
describe('configureDevUserDataPath', () => {
  ...
  it('moves dev runs onto a Live Collab dev userData path', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    ...
    // Why: production code uses path.join(app.getPath('appData'), 'IBM Bob Live Collab Dev')
    // which produces platform-specific separators.
    expect(app.setPath).toHaveBeenCalledWith('userData', join('/tmp/app-data', 'IBM Bob Live Collab Dev'))
  })
