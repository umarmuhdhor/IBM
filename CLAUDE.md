...
- Git flow (PLAN.md §6, PROMPT steps 11/13): work on your lane branch. After each phase: commit, `git tag -f lane-<x>-fNN`, push a snapshot branch `git push origin HEAD:refs/heads/lane/<x>-fNN`, and open the PR from that snapshot (`gh pr create --base main --head lane/<x>-fNN`). Keep working on `lane/<x>`. The owner squash-merges when CI is green (PRs touching `radar/packages/common` or `plan/ref` need Alief's approval). After your own PR merges: `git rebase --onto origin/main lane-<x>-fNN lane/<x>` then `git push --force-with-lease origin lane/<x>`. Only phase 00 is committed straight to `main`. Never merge `main` into a lane branch. Never force-push `main` or anyone else's branch.
